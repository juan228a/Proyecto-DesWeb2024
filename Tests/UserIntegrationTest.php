<?php

use PHPUnit\Framework\TestCase;

class UserIntegrationTest extends TestCase
{
    private $conexion;

    protected function setUp():void
    {
        // Configurar la conexión a la base de datos
        $server = "localhost";
        $user = "root";
        $pass = "";
        $db = "projectofinal_db"; // Base de datos de pruebas

        $this->conexion = new mysqli($server, $user, $pass, $db);
        if ($this->conexion->connect_error) {
            die("conexion fallida: " . $this->conexion->connect_error);
        }
    }

    public function testUserSignupAndLogin():void
    {
        // Paso 1: Registrar un nuevo usuario
        $_POST['username'] = 'juan';
        $_POST['password'] = '1234';
        $signupMessage = $this->signup($_POST['username'], $_POST['password']);
        $this->assertEquals('Registro exitoso', $signupMessage);

        // Paso 2: Iniciar sesión con el nuevo usuario
        $loginMessage = $this->login($_POST['username'], $_POST['password']);
        $this->assertEquals('Se logueo correctamente', $loginMessage);
    }

    private function signup($username, $password)
    {
        // Consulta para registrar un nuevo usuario
        $sql = "INSERT INTO usuarios (usuario, password) VALUES (?, ?)";
        $stmt = $this->conexion->prepare($sql);
        $stmt->bind_param("ss", $username, $password);

        if ($stmt->execute()) {
            return "Registro exitoso";
        } else {
            return "Error en el registro";
        }
    }
    public function testSuccessfulLogin():void
    {
        // Definir datos de prueba
        $_POST['username'] = 'oscar';
        $_POST['password'] = '1234';

        // Ejecutar el método de login y capturar el mensaje de retorno
        $mensaje = $this->login($_POST['username'], $_POST['password']);

        // Verificar que se muestre el mensaje de inicio de sesión exitoso o de error
        $this->assertContains($mensaje, ['Se logueo correctamente', 'Error al logearse']);

        // Asegurar que el mensaje esperado se imprima en la consola
        $this->expectOutputString($mensaje);
    }
    
    private function login($username, $password)
    {
        // Consulta para verificar las credenciales
        $sql = "SELECT * FROM usuarios WHERE usuario = ?";
        $stmt = $this->conexion->prepare($sql);
        $stmt->bind_param("s", $username);
        $stmt->execute();
        $resultado = $stmt->get_result();

        // Verificar si se encontró un usuario y comparar contraseñas
        if ($resultado->num_rows > 0) {
            $fila = $resultado->fetch_assoc();
            if ($fila['password'] === $password) {
                return "Se logueo correctamente";
            } else {
                return "Error al logearse";
            }
        } else {
            return "Error al logearse";
        }
    }

   
}

?>
